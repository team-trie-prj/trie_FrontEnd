...
export default function TopNav() {
  ...
  return (
    <nav className="z-30 shrink-0 border-b border-[#151515] bg-black/70 backdrop-blur-[14px]">
      <div className="flex min-h-[70px] flex-wrap items-center gap-x-[18px] gap-y-1 px-[30px] py-2 max-[480px]:gap-x-2.5 max-[480px]:px-4">
        <BrandLogo />
        ...
        <div className="ml-auto flex shrink-0 items-center gap-2">
          ...
          >
            ...
            <span className="max-[480px]:hidden">히스토리</span>
          </button>
          {isAuthed ? (
            <button
              onClick={async () => {
              ...
              className="flex h-10 items-center whitespace-nowrap rounded-[20px] border border-white/30 px-5 text-sm font-semibold transition-colors hover:bg-white/[.06] max-[480px]:px-3.5"
            >
              {user?.nickname} · 로그아웃
            </button>
          ) : (
